...
def plot_solution(t, y_lbl_m2, y_lbl_m1, x_data, y_data_m2, y_data_m1, y_pred, f_path_name, epoch=None):
    ...
    if y_pred is not None:
        plt.plot(t, y_pred[:, 1], label="pred pos m2 (wagon)", color="tab:orange", linestyle="dashed")
    ...
    plt.subplot(2, 1, 2)
    if y_pred is not None:
        plt.plot(t, y_pred[:, 0], label="pred pos m1", color="tab:orange", linestyle="dashed")
    ...
    if epoch is not None:
        ...
        fig_res.savefig(f_path_name + '.png', format='png', dpi=300)
...
def plot_loss(loss_over_epoch, physics_scale, f_path_name, scaled):
    ...
    plt.yscale("log")
    plt.legend()
    fig_s.savefig(f_path_name + '.png', format='png', dpi=300)
